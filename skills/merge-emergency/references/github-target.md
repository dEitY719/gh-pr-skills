...
[ -f "$_SC/functions/gh_host.sh" ] || {
    printf '[gh-pr:merge-emergency] shell-common not found under %s. On Claude Code this is a broken install; on any other harness export CLAUDE_PLUGIN_ROOT=<plugin dir> first.\n' \
        "$_SC" >&2
