...
- Never move the PR card to `Approved` — that column is owned by
  `gh-pr:approve` (dEitY719/dotfiles#1350). Replies and bot reviews are `COMMENTED` and never
  change `reviewDecision`, so promoting on them lands unreviewed PRs in
  `Approved` (dEitY719/dotfiles#1349 regression). The Step 6 `In review` recovery is the only
...
- To mutate PR labels or body, route through `_gh_pr_edit_safe_label` /
  ...
  / `--body-file` silently exits 1 on classic-Projects repos (issue dEitY719/dotfiles#326 Bug B).
  ...
  head-advancing skill uses (dEitY719/dotfiles#1563) — never an inlined DELETE.
...
- **NF-2 — "never self-certify" — is deliberately RELAXED here, on this one
  ...
  - What it replaced: dEitY719/dotfiles#1563/dEitY719/dotfiles#1616 required an **independent** `gh-pr:review`
    ...
    (dEitY719/dotfiles#1627). The user weighed the extra safety net against pipeline
