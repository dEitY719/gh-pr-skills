...
- Bind `TARGET_REPO` **and** `TARGET_HOST` from one and the same remote
  ...
  [ -f "$_SC/functions/gh_host.sh" ] || {
      printf '[gh-pr:review] shell-common not found under %s. On Claude Code this is a broken install; on any other harness export CLAUDE_PLUGIN_ROOT=<plugin dir> first.\n' \
          "$_SC" >&2
