...
| `[remote]` (positional) | Git remote that owns the PR. Non-digit positional (dEitY719/dotfiles#1405). Drives both the `gh` target (host + `--repo`) and the git plumbing: `git fetch <remote>`, `<remote>/$BASE_BRANCH` ranges, `git push -u <remote> HEAD`. Unknown remote → stop with `git remote -v`, never a silent `origin` fallback. | `origin` |
...
`/gh-pr:create 123 upstream` all parse (dEitY719/dotfiles#1405).
